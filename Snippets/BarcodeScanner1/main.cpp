 ...
 */
void displayImg(const Mat& image, const string& name) {
...
string decodedToDigit(const string& code) {
  if (code == "3211")
    return "0";
  else if (code == "2221")
    return "1";
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < imgRoi.cols; i++) {
    ...
    if (c != color) {
      if (color == 0 && unitBarSize == 0)  // First time seeing a black bar
      {
        ...
        if (unitBarSize < 2) unitBarSize = 2;
      ...
      if (unitBarSize > 0) {
        ...
        // 2
        else if ((unitBarSize + slack) < currentBarSize &&
                 currentBarSize <= (2 * (unitBarSize + slack))) {
          currentDecode.append("2");
        }
        // 3
        else if ((2 * (unitBarSize + slack)) < currentBarSize &&
                 currentBarSize < (3 * (unitBarSize + slack))) {
  ...
  if (currentDecode.substr(0, 3).compare("111") == 0) {
    int sum = 0;
    string code = "";
    for (uint i = 3; i < currentDecode.length(); i++) {
    ...
    string code = "";
    finalNumber = "";
    for (uint i = 0; i < currentDecode.length(); i++) {
      sum += currentDecode[i] - 48;
      code.push_back(currentDecode[i]);
      // cout << sum << endl;
      if (sum == 7) {
